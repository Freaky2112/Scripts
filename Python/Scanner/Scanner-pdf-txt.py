...
try:
    ...
    print("  sudo apt-get install python3-pil tesseract-ocr")
    print("  pip3 install --user pytesseract")
...
def get_next_scan_number(directory):
    ...
    existing_files = [f for f in os.listdir(directory) 
                      if f.startswith("scan") and (f.endswith(".txt") or f.endswith(".pdf"))]
    ...
    for filename in existing_files:
        try:
            # Extract number from filename like "scan01.txt" or "scan01.pdf"
            num_str = filename.replace("scan", "").replace(".txt", "").replace(".pdf", "")
...
def scan_document(save_directory=None, device=None):
    """Scan a document and save as both PDF and text file with auto-incrementing filename"""
    ...
    txt_filename = f"scan{scan_num:02d}.txt"
    pdf_filename = f"scan{scan_num:02d}.pdf"
    ...
    print(f"Starting scan... Will save as: {txt_filename} and {pdf_filename}")
    
    try:
        # Build scanimage command
        # --resolution 1200: Max DPI
        ...
        print("Scanning at 1200 DPI, Black & White mode...")
        ...
        img_for_pdf = img.convert('RGB') if img.mode not in ['RGB', 'L'] else img
        img_for_pdf.save(pdf_filepath, "PDF", resolution=100.0)
        print(f"✓ PDF saved: {pdf_filepath}")
        
        # Perform OCR and save as text
        ...
        try:
            ...
            print("Note: PDF was saved successfully, but text extraction failed.")
            print("Tesseract may not be installed. Install with:")
        ...
        print(f"\n✓ Scan complete!")
        print(f"  Next scan will be: scan{scan_num+1:02d}.pdf and scan{scan_num+1:02d}.txt")
        return True
...
def main():
    ...
    print("Settings: 1200 DPI, Black & White, saves as PDF + Text (OCR)")
    print()
